...
def heapify_up(heap: MinHeap, index: int) -> MinHeap:
    ...
    heap_data = heap.data
    if heap_data[index] < heap_data[parent_index]:  # Change comparison for MinHeap
...
def heapify_down(heap: MinHeap, index: int) -> MinHeap:
    ...
    if left < current_size and heap_data[left] < heap_data[smallest]:  # Change comparison for MinHeap
        smallest = left

    if right < current_size and heap_data[right] < heap_data[smallest]:  # Change comparison for MinHeap
        smallest = right
